...
通过飞书开放平台 API 操作飞书文档。支持创建文档、写入 Markdown 内容（含表格）、读取文档、批注管理、权限管理。
...
python3 skills/feishu-docs/scripts/feishu_doc.py write --doc DOCUMENT_ID --mode overwrite --markdown "替换的内容"
```

从文件读取：
```bash
python3 skills/feishu-docs/scripts/feishu_doc.py write --doc DOCUMENT_ID --markdown-file /path/to/content.md
python3 skills/feishu-docs/scripts/feishu_doc.py write --doc DOCUMENT_ID --mode overwrite --markdown-file /path/to/content.md
```

⚠️ **重要**：
- **默认为追加模式**（`--mode append`），不会覆盖已有内容
- 需要替换文档内容时，**必须显式指定 `--mode overwrite`**
- `--mode overwrite` 会先删除文档所有子 block，再写入新内容
- `create-and-write` 不需要 `--mode`（新文档本身为空）
...
需要替换文档全部内容时，使用 `--mode overwrite`：

```bash
# 先准备好完整的新内容文件
python3 skills/feishu-docs/scripts/feishu_doc.py write \
  --doc DOCUMENT_ID \
  --mode overwrite \
  --markdown-file /tmp/updated_content.md
```
